...
//	
//
//  Source file: /afs/.ir.stanford.edu/users/a/d/adgil/EE271/EE271_project/assignment2/rtl/bbox.vp
...
module bbox_unq1
  (
   ...
   //Calculate bounding box using gold code as model

   x_comp_R10S[0] =  poly_R10S[0][0] < poly_R10S[1][0];
   x_comp_R10S[1] =  poly_R10S[0][0] < poly_R10S[2][0];
   x_comp_R10S[2] =  poly_R10S[1][0] < poly_R10S[2][0];

   y_comp_R10S[0] =  poly_R10S[0][1] < poly_R10S[1][1];
   y_comp_R10S[1] =  poly_R10S[0][1] < poly_R10S[2][1];
   y_comp_R10S[2] =  poly_R10S[1][1] < poly_R10S[2][1];
   // poly_R10S[vert][axis]

   case (x_comp_R10S)
      3'b11x:  box_R10S[0][0] = poly_R10S[0][0];
      3'b0x1:  box_R10S[0][0] = poly_R10S[1][0];
      3'bx00:  box_R10S[0][0] = poly_R10S[2][0];
      default: box_R10S[0][0] = poly_R10S[0][0]; // should never occur
   endcase

   case (x_comp_R10S)
      3'b00x:  box_R10S[1][0] = poly_R10S[0][0];
      3'b1x0:  box_R10S[1][0] = poly_R10S[1][0];
      3'bx11:  box_R10S[1][0] = poly_R10S[2][0];
      default: box_R10S[1][0] = poly_R10S[0][0]; // should never happen
   endcase

   case (y_comp_R10S)
      3'b11x:  box_R10S[0][1] = poly_R10S[0][1];
      3'b0x1:  box_R10S[0][1] = poly_R10S[1][1];
      3'bx00:  box_R10S[0][1] = poly_R10S[2][1];
      default: box_R10S[0][1] = poly_R10S[0][1]; // should never occur
   endcase

   case (y_comp_R10S)
      3'b00x:  box_R10S[0][1] = poly_R10S[0][1];
      3'b1x0:  box_R10S[0][1] = poly_R10S[1][1];
      3'bx11:  box_R10S[0][1] = poly_R10S[2][1];
      default: box_R10S[0][1] = poly_R10S[0][1]; // should never occur
   endcase
   ...
   // box_R10S[1][1]: UR Y
      ...
      if (rounded_box_R10S[0][0] < 0) out_box_R10S = 0;
      if (rounded_box_R10S[0][1] < 0) out_box_R10S = 0;
      if (rounded_box_R10S[1][0] > screen_RnnnnS[0]) out_box_R10S = screen_RnnnnS[0];
      if (rounded_box_R10S[1][1] > screen_RnnnnS[1]) out_box_R10S = screen_RnnnnS[1];

      if (rounded_box_R10S[0][0] < screen_RnnnnS[0] &&
          rounded_box_R10S[1][0] > 0 &&
          rounded_box_R10S[0][1] < screen_RnnnnS[1] &&
          rounded_box_R10S[1][1] > 0) 
         outvalid_R10H = 1;
      else outvalid_R10H = 0;
   end
